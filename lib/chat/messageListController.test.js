...
describe('Chat.messageListController - unit tests', () => {
  ...
  describe('methods', () => {
    ...
    beforeEach(() => {
      originalAddMessage = Chat.messageListController.addMessage
    ...
    afterEach(() => {
      Chat.messageListController.addMessage = originalAddMessage
    })
    ...
    describe('setModel method', () => {
      ...
      it('should observe with bound addMessage', () => {
        const stub = Chat.messageListController.addMessage = stubFn()

        this.controller.setModel(this.model) // do not remove
        this.model.observe.args[1]()

        assert(stub.called)
        assert.strictEqual(stub.thisArg, Chat.messageListController)
      })
